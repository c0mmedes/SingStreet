...
const Studio = () => {
  ...
  useEffect(()=>{
    const init = async ()=> {
        ...
        // const blockList = getBlockList(); -> 블럭 가져오기(서버에서도 음원 가져와)
        ...
        const dragStartHandler = e =>{
            ...
            myMap.set(blockId, {left:left, top:top});
        }
    ...
    //통신을 통해 가져온 음원파일 blocksetting
    // blockList.forEach(block.blockId){
    //     setblock(block.blockId);
    // }
    setblock("item1");
    setblock("item2");

    myMap.observe(()=>{
        myMap.forEach((block, blockId) =>{
            const blockElement = document.getElementById(blockId);
            if(blockElement){
                setLocation(blockElement, block.left, block.top);
            }
    ...
    <div className='studioArea' onDrop={handleDrop} onDragOver={(e) => e.preventDefault()}>
        <div className='inner'>
            <div className='buttonArea'>
                ...
                <button id='stop' onClick={handleStop} className='w-btn-neon2'>stop!</button>
