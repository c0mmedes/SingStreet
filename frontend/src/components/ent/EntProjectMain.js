...
const EntProjectMain = ({ userInfo }) => {
	...
	const onClickGoToProjectMemberInvite = () => {
		...
		if (parseInt(project.userId) === parseInt(userInfo.userId)) {
			// 프로젝트 멤버 초대로 이동
			navigate(`/entmain/${entId}/${entMasterId}/${entName}/entprojectmemberinvite/${projectId}`);
		} else {
			alert("권한이 없습니다!");
		}
	};
	...
	return (
		<div className="pjtMainWrap">
			...
			<div className="pjtClick">
				<div className="pjtClickbtn pjtClickbtnDelete" onClick={onClickDeleteProject}>
					삭제
				</div>
				<div className="pjtClickbtn pjtClickbtnModify" onClick={onClickGoToProjectModify}>
					수정
				</div>
				<div className="pjtClickbtn pjtClickbtnModify" onClick={onClickGoToProjectMemberInvite}>
					멤버 초대
				</div>
			</div>
		</div>
